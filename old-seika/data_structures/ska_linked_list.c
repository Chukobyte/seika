...
SkaLinkedList* ska_linked_list_create(size_t valueSize) {
    SkaLinkedList* list = SE_MEM_ALLOCATE(SkaLinkedList);
...
void* ska_linked_list_pop(SkaLinkedList* list, size_t index) {
    if (list->size > 0 && index < list->size) {
        ...
        SE_ASSERT(nodeBeforePopped->next);
...
static SkaLinkedListNode* linked_list_create_node(SkaLinkedList* list, void* value, SkaLinkedListNode* next) {
    SkaLinkedListNode* node = SE_MEM_ALLOCATE(SkaLinkedListNode);
